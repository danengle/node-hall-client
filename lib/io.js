...
var _ = require('underscore'),
  ...
  streamBaseURL = function(session) {
    console.log({
      ...
      md5: crypto.createHash('md5').update("" + session.get('uuid') + session.get('authenticity_token')).digest("hex"),
      url: url.parse((process.env.NODE_HALL_STREAMING_URL || 'https://stream.hall.com')+'/socket.io/1/?user_session_id='+session.get('user_session_id')+'&id='+session.get('uuid')+'&session='+crypto.createHash('md5').update("" + session.get('uuid') + session.get('authenticity_token')).digest("hex"))
    });
  ...
  IO = Module.extend({
    ...
    _connect : function(clearTimeout) {
        ...
        var url = streamWSURL(me, me.session).href;
        var baseUrl = baseURL().href
        var headers = _.extend({}, HallUserAgent.getHeader(), { cookie : me.session.get('cookie') });
        ...
        me.client.connect(url, null, baseUrl, { headers : headers });
